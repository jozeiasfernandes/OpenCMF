...
class Janela2D(JanelaBase):
    ...
    def __init__(self, titulo: str, cor: str, parent=None, scene_manager: Optional["SceneManager"] = None):
...
def create_component(context: Any) -> Janela2D:
    scene_manager = getattr(context, "scene_manager", None)
    return Janela2D(titulo="Visor 2D", cor="#202020", scene_manager=scene_manager)
...
if __name__ == "__main__":
    ...
    main_window.setWindowTitle("Teste Janela2D")
    janela = Janela2D("Visor 2D", "#202020")
    main_window.setCentralWidget(janela)
