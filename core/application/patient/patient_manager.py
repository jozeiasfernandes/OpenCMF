...
class PatientManager(QtCore.QObject):
    ...
    @classmethod
    def get_instance(cls, project_service: Any = None) -> PatientManager:
        """Retorna a instância única do PatientManager."""
        if cls._instance is None:
            if project_service is None:
                raise ValueError("Um serviço de projeto válido é obrigatório na primeira inicialização do PatientManager.")
            cls(project_service)
    ...
    def current_path(self) -> Optional[str]:
        """Retorna o path absoluto do paciente ativo."""
    ...
    # PUBLIC METHODS
    # =========================================================================
    def set_active_patient(self, path: str | Path):
        ...
        # Proteção contra múltiplos carregamentos idênticos em sequência rápida (Throttling)
        ...
        if self.project_service and hasattr(self.project_service, "load_project"):
            loaded_data = self.project_service.load_project(Path(resolved_path))
            self._project_data = loaded_data if loaded_data is not None else {}
        else:
            self._project_data = {}

        # Emite os sinais na ordem lógica
    ...
    def save_current_data(self):
        ...
        if hasattr(self.project_service, "save_project"):
