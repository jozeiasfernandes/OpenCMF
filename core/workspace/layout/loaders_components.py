...
class WorkspaceComponentHandler:
    ...
    def _on_componente_configurado(self, categoria: str, caminho: Path, ativo: bool):
        ...
        modulo_ativo = self.workspace.get_active_module()
        if not modulo_ativo:
    ...
    def _carregar_e_injetar(self, categoria: str, caminho: Path, modulo_ativo):
        try:
            ...
            # Ajustado para corresponder à assinatura do ComponentLoader (path, context)
            comp = ComponentLoader.load(caminho, modulo_ativo)
            if not comp:
                return
